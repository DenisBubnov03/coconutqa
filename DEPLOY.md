...
## Визуальное обновление — сентябрь 2026

Новый первый экран находится в `index.html`, стили и эффекты — в
`index_files/coconut-motion.css` и `index_files/coconut-motion.js`.
Оригинальный первый экран Tilda сохранён, но скрыт стилями. Остальные блоки,
попапы и `face-page-fixes.*` продолжают работать как раньше.

Эффекты: плавное покачивание персонажа, параллакс портрета и персонажа от
указателя, однократное появление секций при прокрутке, индикатор чтения,
состояния наведения и клавиатурного фокуса. При `prefers-reduced-motion`
покачивание, параллакс и появление отключаются; на сенсорных устройствах
параллакс отключён. Новых библиотек и этапа сборки нет.
